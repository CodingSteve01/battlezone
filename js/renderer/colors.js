// ===== COLOR UTILITIES =====
// Safe color operations, gradients, and color manipulation
...
/**
 * Apply lighting to a color based on light factor
 */
export function applyLightingToColor(color, lightFactor) {
    const num = parseInt(color.replace('#', ''), 16);
    let r = (num >> 16) & 0xFF;
    let g = (num >> 8) & 0xFF;
    let b = num & 0xFF;

    r = Math.round(Math.min(255, r * lightFactor));
    g = Math.round(Math.min(255, g * lightFactor));
    b = Math.round(Math.min(255, b * lightFactor));

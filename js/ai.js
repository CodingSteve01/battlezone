...
const SPECTATOR_PACE = {
    unitDelay: 900,
    actionDelayBase: 350,
    shortDelay: 180,
    scrollDelay: 350,
    targetingDelay: 280,
    afterAttackDelay: 420,
    stepDelay: 120,
    scrollDuration: 250,
    turnEndDelay: 450
};
...
async function performAIActions() {
    ...
    const unitDelay = spectatorMode ? SPECTATOR_PACE.unitDelay : 400;
    ...
    try {
        ...
        setTimeout(() => {
            endTurn();
        }, spectatorMode ? SPECTATOR_PACE.turnEndDelay : 500);
    }
}
...
async function performUnitAI(unit, plan, spectatorMode = false) {
    ...
    try {
        ...
        const actionDelayBase = spectatorMode ? SPECTATOR_PACE.actionDelayBase : 300;
        const shortDelay = spectatorMode ? SPECTATOR_PACE.shortDelay : 100;
...
async function executeDecoyBehavior(unit, plan, renderIfVisible, hasHumanViewer, spectatorMode = false) {
    const enemies = plan.visibleEnemies;
    const unitName = CLASS_NAMES_DE[unit.class] || unit.class || 'Einheit';
    const actionDelay = spectatorMode ? SPECTATOR_PACE.actionDelayBase : 300;
...
async function executeAttackSequence(unit, target, renderIfVisible, hasHumanViewer, spectatorMode = false) {
    ...
    const scrollDelay = spectatorMode ? SPECTATOR_PACE.scrollDelay : 400;
    const targetingDelay = spectatorMode ? SPECTATOR_PACE.targetingDelay : 300;
    const afterAttackDelay = spectatorMode ? SPECTATOR_PACE.afterAttackDelay : 500;
    const shortDelay = spectatorMode ? SPECTATOR_PACE.shortDelay : 100;
...
async function executeAIMove(unit, target, spectatorMode = false) {
    ...
    const stepDelay = spectatorMode ? SPECTATOR_PACE.stepDelay : 120;
    const scrollDuration = spectatorMode ? SPECTATOR_PACE.scrollDuration : 200;

...
// Enhanced with threat maps, predictions, and look-ahead capabilities
...
import {
    ...
    findAttackOpportunities,
    calculateExposure,
    findCrossfirePositions
} from './ai/threats.js';
import {
    predictEnemyMovement,
    predictAttackTargets,
    simulateEnemyTurn,
    identifyUnitsAtRisk,
    findBaitPositions,
    calculatePostAttackSafety
} from './ai/prediction.js';
import {
    getAIMemory,
    setAIMemory,
    createTeamMemory,
...
// Decoy/bait strategy functions imported from ./ai/decoy.js:
// getEnemySpawnCenters, shouldUseDecoyStrategy, findDecoyCandidate, planDecoyStrategy,
// isDecoyUnit, isAmbushUnit, scoreDecoyPosition, scoreAmbushPosition
...
function analyzeAndPlan() {
    ...
    // This is the key to making AI "think ahead"
    const aliveAiUnits = aiUnits.filter(u => u.alive);
    calculateThreatMap(visibleEnemies, aliveAiUnits);
    calculateInfluenceMap(visibleEnemies, aliveAiUnits);

    // === NEW: PREDICT ENEMY MOVEMENTS ===
    const enemyPredictions = [];
    for (const enemy of visibleEnemies) {
        const predictions = predictEnemyMovement(enemy, aliveAiUnits);
        if (predictions.length > 0) {
            enemyPredictions.push({
                enemy,
                likelyPosition: predictions[0],
    ...
    const unitsAtRisk = identifyUnitsAtRisk(aliveAiUnits, visibleEnemies);
    const criticalUnits = unitsAtRisk.filter(r => r.inMortalDanger);

    // Warn about endangered units
    if (criticalUnits.length > 0 && visibleEnemies.length > 0) {
        const unitNames = criticalUnits.map(r => CLASS_NAMES_DE[r.unit.class] || r.unit.class).join(', ');
        addAIThought(`⚠️ WARNUNG: ${unitNames} in Lebensgefahr! Schutzmaßnahmen eingeleitet.`, 'strategy');
    }

    // Store predictions in memory for use by positioning functions
    aiMemory.enemyPredictions = enemyPredictions;
    aiMemory.unitsAtRisk = unitsAtRisk;
...
async function executeRetreat(unit, enemies, spectatorMode = false) {
    ...
    // === NEW: USE THREAT MAPS FOR SMARTER RETREAT ===
    // Try to find the safest reachable hex using threat analysis
    const safestHex = findSafestReachableHex(unit, enemies);
    if (safestHex && safestHex.threat < getThreatAt(unit.q, unit.r) * 0.5) {
        // Found a much safer position - use it!
        await executeAIMove(unit, safestHex, spectatorMode);
        return;
    }

    // Fallback: Original retreat logic with improvements
    const reachable = getReachableHexes(unit);
    if (reachable.size === 0) return;
    ...
    reachable.forEach((data, key) => {
        ...
        // === NEW: USE THREAT MAP ===
        const threatLevel = getThreatAt(q, r);
        score -= threatLevel * 1.5; // Heavy penalty for dangerous positions
        ...
        // Prefer cover (MORE important when retreating)
        if (hex.cover) score += 50;
        ...
        if (allies.length > 0) {
            ...
            score += (10 - closestAlly) * 8; // Increased weight

            // Extra bonus for moving near a medic
            const nearbyMedic = allies.find(a =>
                a.class === 'medic' &&
                hexDistance({ q, r }, { q: a.q, r: a.r }) <= 4
            );
            if (nearbyMedic) score += 40;
        }
 ...
 * VERBESSERTE KI: Klassenspezifische Positionierung + Threat Maps
 */
function scoreCombatPosition(unit, q, r, enemies, plan) {
    ...
    // === NEW: THREAT MAP INTEGRATION ===
    // This is the KEY improvement - AI now considers danger levels
    const threatLevel = getThreatAt(q, r);
    ...
    if (influence && influence.control > 0) {
        score += influence.control * 5;
    }

    // === NEW: CHECK POST-ATTACK SAFETY ===
    // Don't overextend! Check if we'll survive after attacking
    const allAllies = getAllAlliedAIUnits();
    ...
    if (primaryTarget) {
        ...
        if (distToTarget <= unit.range) {
            ...
            // Don't overextend! Check if we'll survive counter-attacks
            const safety = calculatePostAttackSafety(
                unit, primaryTarget, q, r, enemies, allAllies
            );

            if (safety.isDangerous) {
                // Position is too dangerous - heavy penalty
                score -= 100;
            } else if (safety.isSafe) {
                // Safe attack position - bonus!
                score += 40;
            } else if (safety.counterAttackers >= 2) {
                // Multiple enemies can counter - moderate penalty
                score -= 50;
            }

            // Kill shot bonus - worth taking more risk for kills
            if (primaryTarget.currentHp <= unit.damage) {
                score += 60; // Increased for guaranteed kills
            }
        }
    }
...
function scoreHuntPosition(unit, q, r, plan) {
    ...
    // === NEW: USE PREDICTED POSITIONS (HIGHER PRIORITY) ===
    // Predictions are smarter than last-known positions
    const predictions = aiMemory.enemyPredictions || [];
    if (predictions.length > 0) {
        // Move toward highest-confidence prediction
        for (const pred of predictions) {
            if (pred.likelyPosition && pred.likelyPosition.probability > 0.3) {
                const distToPredicted = hexDistance({ q, r }, {
                    q: pred.likelyPosition.q,
                    r: pred.likelyPosition.r
                });

                // Strong pull toward predicted position
                score += (12 - distToPredicted) * 30;

                // Bonus for being in attack range of predicted position
                if (distToPredicted <= unit.range) {
                    score += 100; // Higher than last-known because it's predicted
                }
            }
        }
    }
